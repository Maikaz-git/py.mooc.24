...
def most_goals(team: list):
    return sorted(team, key=lambda ply: ply.goals)[-1].name

def most_points(team: list)-> tuple:
    res= sorted(team, key=lambda ply:ply.goals + ply.passes)[-1]
    return (res.name, res.number)

def least_minutes(team: list):

    return sorted(team , key=lambda ply:ply.minutes)[0]
